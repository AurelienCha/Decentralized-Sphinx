...
\begin{tikzpicture}
    ...
    \vgap
    \node (s3_) [block=5] at (0, \y) {};
    \node [block=1] at (0, \y) {\sG{3}{1}};
    \foreach \j in {1,...,4} {\node [zero_pad=1] at (\j*\width, \y) {0};}
    \vgap
    \node (s2_) [block=5] at (0, \y) {};
    \node [zero_pad=1] at (0, \y) {0};
    \foreach \j in {4,...,7} {
        \node [block=1] at (\j*\width - 3*\width, \y) {\sG{2}{\j}}; 
    }
    \vgap
    \node [zero_pad=1] at (0, \y) {0}; 
    \node (s1_) [block=5] at (0, \y) {};
    \foreach \j in {6,...,7} {
        \node [block=1] at (\j*\width - 5*\width, \y) {\sG{1}{\j}}; 
    }
    ...
    \node (xor3) [eq] at (-\width, \y) {\ref{eq:last_layer}}; 
    % \node (xor3) [eq] at (-5*\width, \y) {\ref{eq:last_layer}}; 
    \vGap
    ...
    \draw[arrow] (s3_.west) -- ++(-\width, 0) -- (xor3.north);
    ...
    \node (bot) at (n1.south) {};
    \node (left) at ($(xor2.west) - (\width/3, 0)$) {};
    \node (right) at ($(zero1.east) + (0.1*\width, 0)$) {};

    \draw[densely dotted] ($(middleTop -| left) - (1, 0)$) -- ($(middleTop -| right) + (1.5, 0)$) {};
    \draw[densely dotted] ($(middleBot -| left) - (1, 0)$) -- ($(middleBot -| right) + (1.5, 0)$) {};
    \node[between=top and middleTop] (l3){};
    \node[between=middleTop and middleBot] (l2){};
    \node[between=middleBot and bot] (l1){};
    \node[xshift=10mm, rotate=90] at (l3 -| right) {\color{black!40} Last layer};  
    \node[xshift=10mm, rotate=90] at (l2 -| right) {\color{black!40} Middle layer};  
    \node[xshift=10mm, rotate=90] at (l1 -| right) {\color{black!40} First layer}; 
