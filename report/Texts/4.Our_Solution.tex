...
In our approach to decentralized Sphinx header construction, each piece of header information is encoded as an elliptic curve point. 
...
However, this approach could leak information by introducing bias since nearby integers result in nearby points.

To address this, we adopt Elligator~\cite{bernstein2013elligator}, which offers stronger privacy guarantees. 
Unlike the traditional approach, Elligator produces a uniformly distributed output which is computationally indistinguishable from a random curve point. 
...
\begin{figure}[t]
    ...
    (2) Secrets are split into shares and sent to TTPs. 
    (3) Each TTP constructs a partial header. 
    (4) The client aggregates partial headers into a final header. 
...
(1) The client first computes a sequence of shared secrets and then splits these secrets, along with IP addresses of the mixnodes in the path and the final destination, into $m$ shares.  
(2) Each set of shares is sent to $m$ different TTP, along with the necessary cryptographic element (\A{}).  
(3) Each TTP independently computes a \textit{partial} Sphinx header using the received shares.  
...
\begin{itemize}
    \item \textbf{Setup}: selects random generators (once but refreshable);
...
(see security note in \ref{note:security_why_indep_generators}).
...
However, to preserve its security, the set of generators should be refreshed periodically. 
...
We simply increment the seed to produce \textit{independent} generators since Elligator already provides a uniform (i.e. pseudo-random) integers to points mapping.

A subtlety with Elligator is that it may map integers to points outside of the base generator's subgroup.
...
These IP points and the shared secrets are then split into shares and distributed to $m$ different TTPs, along with a hash of each secret. % for integrity
...
To construct the partial encrypted header, each TTP receives the following information from the client:
\begin{itemize}
...
Using these data, the TTP builds a partial encrypted header by layering the routing information in reverse, starting from the last mixnode, as illustrated in Figure~\ref{fig:chunked_schema}.
...
Since TTPs are aware of these pseudo-random weights, omitting this offset would allow TTPs to potentially link some incoming and outgoing mixnode's packets by simply verifying integrity tags.
\begin{equation}
...
Finally, the TTP returns the resulting partial header to the client.

\begin{figure}[H]
    \centering
    \resizebox{1.\linewidth}{!}{\input{schema/chunkwise_encryption.tex}}
    \caption{Chunk-wise header encryption done by the TTP. Circled numbers \eq[x] refers to Equation x in the paper.}
