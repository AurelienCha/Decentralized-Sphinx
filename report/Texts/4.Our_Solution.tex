...
(1) The client first computes a sequence of shared secrets and then splits these secrets, along with the IP addresses of the mixnodes in the path and the final destination, into $m$ shares.  
(2) Each set of shares is sent to a different TTP, along with the necessary cryptographic element (\A{}).  
(3) Each TTP independently computes a \textit{partial} Sphinx header using the received shares.  
(4) The client then aggregates these partial headers to reconstruct the final header, (5) ready for transmission through the mixnet.
...
In addition, the client sends the hash of each shared secret to these TTPs. % for integrity
