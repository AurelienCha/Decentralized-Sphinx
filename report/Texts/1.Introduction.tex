...
During the last few decades several systems have been proposed in the literature and few of them have been implemented. Open research problems such as parameterization, authentication and fake dummies have halted the usage on a large scale.\footnote{cite}
%
\todo{Also this speculation, needs rephrasing}
\todo[inline]{what are \enquote{fake dummies} do we still say \enquote{dummies} :-s}
%
...
certificate of paying for a specific service to use the Nym network.\footnote{we do not need to stick to nym that now we are submitting to wpes, Sphinx is used by various ACNs}
However, nothing prevents users from cheating who might exploit a valid Nym
%
\todo[inline]{Can we quantify the impact somehow? Show that this is a real problem?}
%
...
\todo[inline]{Would there be a risk of unauthenticated attacker-controlled traffic
...
\end{itemize}
%
\todo[inline]{Something on prototype and availability of artifacts!}
\todo[inline]{For CBT: highlight relevance of the contribution to Lightning and Nym
