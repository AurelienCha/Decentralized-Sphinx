\section{Discussion}\label{sec:discussion} % We regard these directions as crucial next steps toward a practical, secure, and scalable deployment of decentralized Sphinx-based mix networks.
...
While our construction successfully decentralizes the Sphinx header computation, our design remains \emph{client-centric} in several key aspects.  
The client still selects the path of a message. A malicious client can still bias route selection to reduce the anonymity set of honest users, or orchestrate classical deanonymization attacks, including (N-1)-attacks, by carefully crafting routes.  
...
Deciding on how to choose the the different nodes in the path will depend on the sepecefitie of the system. For example in the lightening network the path is chosen using Djikstra, in Nym the path is chosen uniformly. Therefore more work is needed to distribute path selection and key derivation to STTPs in a way that matches the systems specefics.
...
A major challenge in bringing decentralized networks such as mixnes and peer-to-peer into practical use is the lack of accountability and policy-enforcement mechanisms that do not undermine privacy~\cite{oram2001peer}.
