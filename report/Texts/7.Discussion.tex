...
The client still selects the path of a message. A malicious client can still bias route selection to reduce the anonymity set of honest users, or orchestrate classical deanonymization attacks, including (N-1)-attacks, by carefully crafting routes~\cite{oram2001peer}.  
...
Deciding how to choose the nodes in the path depends on the characteristics of the underlying system. For example, in the Lightning Network the path is selected using Dijkstra’s algorithm~\cite{poon2016bitcoin}, while in Nym every mixnode in the path is chosen uniformly from all the mixnodes in the corresponding layer~\cite{nym-network-whitepaper}. Therefore, in order to distribute path selection and key derivation to the STTPs, DSphinx must be aligned with the design of the specific system. Further work is thus required to ensure that path selection and key derivation are distributed to the STTPs in a manner that matches the requirements of each system.

\noindent\textbf{Collusion between STTPs and Hops.} 
Our security analysis considers several collusion scenarios involving corrupted STTPs and hopd.  
The current scheme guarantees security as long as (i) at least one STTP remains honest and (ii) not all hops on the path are compromised.  
However, a subtle but significant limitation arises when an adversary simultaneously compromises a hop along the path and one of the STTPs that contributed to the packet construction.  
In this case, the adversary can associate the integrity weights leaked by the STTP with the specific packet observed at the corrupted hop.  
...
Future work should investigate stronger homomorphic integrity mechanisms or alternative constructions of the checksum that do not reveal correlation between weights of honest hops, even under collusion.
