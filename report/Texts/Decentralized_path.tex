...
In our approach, mixnode public keys are no longer used for path construction. 
Instead, each mixnode's private key $k_i$ is distributed among STTPs using Shamir Secret Sharing, resulting in a distributed symmetric key shared among the STTPs.
...
Upon receiving a nonce from the client, the STTPs collaboratively sample a path in a pseudorandom manner and return shares of the corresponding secret material. 
After collecting a threshold number of responses, the client reconstructs the shared secrets and proceeds with the DSphinx header construction procedure (Fig.~\ref{fig:overall_schema}).
