\begin{frame}
    \begin{enumerate}
        \item Introduction
        \begin{itemize}
            \item Motivation - Speak about importance of Privacy on the network
            \item Some Technologies: TOR vs Mixnet {\color{gray}(lot of animations)}
        \end{itemize}
        \item The problem
        \begin{itemize}
            \item Mixnet's problem (discard illegal packet at exit gateway)
            \item Our idea - Decentralization
        \end{itemize}
        \item Sphinx
        \begin{itemize}
            \item Original Sphinx Processing    {\color{gray}(lot of animations)}
            \item Original Sphinx Construction  {\color{gray}(lot of animations)}       {\color{orange}{(in appendix for conference presentation)}}
            \item ECC slide                                                             {\color{orange}{(in appendix for conference presentation)}}
            \item Our Sphinx implementation     {\color{gray}(lot of animations)}
        \end{itemize}
        \item Tests and Results
        \item Conclusion and future work
    \end{enumerate}
\end{frame}