...
\only<\thebeamerpauses>{ 
    ...
    {\color{highlight}\at{B31}{-1}{1.4}               (salt3)                             {$(\s{3}, \sHashed{3})$};}
    \node at (salt3-|B22)           (HMAC3)            [HMAC]           {\tiny HMAC};
} \only<\thebeamerpausesplusTWO->{\at{B31}{-1}{1.4}               (salt3)                             {$(\s{3}, \sHashed{3})$};
} \only<\thebeamerpausesplusTWO->{\node at (salt3-|B32)           (HMAC3)            [eq] {5};}
...
\only<\thebeamerpauses>{
    ...
    {\color{highlight} \at{B21}{-1}{1.4}               (salt2)                             {$(\s{2}, \sHashed{2})$};}
    \node at (salt2-|B22)           (HMAC2)            [HMAC]           {\tiny HMAC};
} \only<\thebeamerpausesplusTWO->{
    \at{B21}{-1}{1.4}               (salt2)                             {$(\s{2}, \sHashed{2})$};
} \only<\thebeamerpausesplusTWO->{\node at (salt2-|B22)           (HMAC2)            [eq] {5};}
...
\only<\thebeamerpauses>{
    ...
    {\color{highlight} \at{B11}{-1}{1.4}               (salt1)                             {$(\s{1}, \sHashed{1})$};}
    \node at (salt1-|B12)           (HMAC1)            [HMAC]           {\tiny HMAC};
} \only<\thebeamerpausesplusTWO->{
    \at{B11}{-1}{1.4}               (salt1)                             {$(\s{1}, \sHashed{1})$};
} \only<\thebeamerpausesplusTWO->{\node at (salt1-|B12)           (HMAC1)            [eq] {5};}
...
\renewcommand{\size}{\small}
\renewcommand{\subsize}{\tiny}
\only<\thebeamerpauses->{
\at{IP}{-8}{2} [draw=red!70!black, fill=red!10, rounded corners=4pt, thick, inner sep=7pt,text width=6.6cm, align=left] 
    (warn) {\small
    \textbf{\Warning \hfill \scalebox{1.5}{\raisebox{1.5mm}{$\s{\hspace{0.5pt}\textrm{\i}} \neq \hash{\sHashed{\textrm{\i}}}$}} \hfill \Warning}
    \vspace{-2mm}
    $\begin{aligned}
        \scalebox{1.5}{\sHashed{\i}}         &: &\textbf{hash}  &\text{ of shared secret with mixnode i}\\
        \scalebox{1.5}{\s{\hspace{0.5pt}i}} &: &\textbf{share} &\text{ of shared secret with mixnode i}   
    \end{aligned}$
    };
}
...
\only<\thebeamerpausesplusONE>{
    \at{HMAC2}{-8.5}{0} (hmacFormula) [draw,anchor=base] {$\C{\i} = \sG{\hspace{0.5pt}i}{} + \sum_{j=1}^{5} \big( \hashIt{\sHashed{\i}}{j} \, \B{\i}{j} \big)$};
    \node [eq] at (hmacFormula.north west) {5};
}