...
- The curve is defined over a 'finite field' Z_p (i.e. mod p)
- The 'curve order' is the total number of valid points (including the point at infinity) on the elliptic curve over the finite field.
- The 'subgroup order' is the number of points in the subgroup accessible from the generator (i.e. N)
- The 'cofactor' is the ratio between the curve order and the subgroup order (i.e. h) => I.e. number of subgroup on the curve

NOTE:
Most of ECC library (like ecpy) use the term 'curve order' instead of 'subgroup order'.
This means that the total number of points on the curve in python is: #E = h * N = cofactor * curve.order
"""
...
N = curve.order                         # nbr points in the subgroup generated by G (i.e. N*G = O, (N+1)*G = G)
cofactor = int(p/N)                     # I.e. number of subgroup on the curve
