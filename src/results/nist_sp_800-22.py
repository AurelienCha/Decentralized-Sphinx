...
def block_frequency_test(bits: str, block_size: int = 32) -> float:
    ...
    p_value = chi2.sf(4 * block_size * chi_sq, df=N)
    return p_value
...
def runs_test(bits: str) -> float:
    ...
    """
    n = len(bits)
    pi = sum(1 if b == '1' else 0 for b in block) / n
...
def cumulative_sums_test(bits: str) -> float:
    ...
    z = max(S) 
    p_value = 2 * math.erfc(z / math.sqrt(2*n)) # not the exact formula as NIST but good approx
    return p_value
